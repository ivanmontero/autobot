...
AutoencoderEncoderOut = NamedTuple(
    "EncoderOut",
    [
...
class HuggingfaceEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        if hasattr(args, 'bert_from_scratch') and args.bert_from_scratch:
            ...
            config.num_attention_heads = args.encoder_attention_heads
            config.intermediate_size = args.encoder_ffn_embed_dim
            model_for_mlm = AutoModelForMaskedLM.from_config(config)
        else:
...
class AutoencoderDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.layers.extend(
            [
                self.build_decoder_layer(args, no_encoder_attn or self.cls_input)
    ...
    def build_decoder_layer(self, args, no_encoder_attn=False):
        return AutoencoderDecoderLayer(args, no_encoder_attn)

    def forward(
        self,
        prev_output_tokens,
        encoder_out: Optional[AutoencoderEncoderOut] = None,
